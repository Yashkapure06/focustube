        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                case 'getClips':
                    ...
                    break;
                case 'saveTimestamp': {
                    const clips = this._context.globalState.get('focustube.clips', []);
                    const clip = {
                ...
                case 'deleteClip': {
                    let clips = this._context.globalState.get('focustube.clips', []);
...
FocusTubeViewProvider.viewType = 'focustube.sidebar';
function activate(context) {
    const provider = new FocusTubeViewProvider(context.extensionUri, context);
    // Register the sidebar webview view
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(FocusTubeViewProvider.viewType, provider, { webviewOptions: { retainContextWhenHidden: true } }));
    // "FocusTube: Open" — focus the sidebar panel
    context.subscriptions.push(vscode.commands.registerCommand('focustube.open', () => {
        vscode.commands.executeCommand('focustube.sidebar.focus');
    }));
    // "FocusTube: Save Timestamp" — delegate to provider
    context.subscriptions.push(vscode.commands.registerCommand('focustube.saveTimestamp', () => {
        provider.triggerSaveTimestamp();
    }));
    // "FocusTube: Show Saved Clips" — scroll to clips list
    context.subscriptions.push(vscode.commands.registerCommand('focustube.showSavedClips', () => {
